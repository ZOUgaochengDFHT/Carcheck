...
public class SKPhotoBrowser: UIViewController, UIScrollViewDelegate, UIActionSheetDelegate {
    ...
    func tapAction(tap:UITapGestureRecognizer) {
        let tagIndex = (tap.view?.tag)! - 500
        let generalDetailView = ZGCGeneralDetailView(frame: CGRectZero, content: self.attri2DArr[tagIndex] as! String)
        ...
        generalDetailView.sureOrCancelHandler = {
            ...
            contentLabel.text = attriArr.lastObject as? String
            bgView.hidden = false
        }
    }
    ...
    public func setControlsHidden(hidden: Bool, animated: Bool, permanent: Bool) {
        ...
        UIView.animateWithDuration(0.35,
            animations: { () -> Void in
                ...
                let bgView = self.pagingScrollView.viewWithTag(500 + self.currentPageIndex)
                bgView?.frame = hidden ? self.frameForBgViewHideAtOrientation() : self.frameForBgViewAtOrientation()
                self.toolBar.frame = hidden ? self.frameForToolbarHideAtOrientation() : self.frameForToolbarAtOrientation()
    ...
    public func frameForBgViewAtOrientation() -> CGRect {
        return CGRect(x: 0, y: view.bounds.size.height - 140, width: view.bounds.size.width + 20, height: 80)
    }
    
    public func frameForBgViewHideAtOrientation() -> CGRect {
        let currentOrientation = UIApplication.sharedApplication().statusBarOrientation
        var height: CGFloat = navigationController?.navigationBar.frame.size.height ?? 44
        if UIInterfaceOrientationIsLandscape(currentOrientation) {
            height = 32
        }
        return CGRect(x: 0, y: view.bounds.size.height + height, width: view.bounds.size.width + 20, height: 80)
    }
