...
class ZGCImageDBManager: NSObject {
    let dbPath:String
    ...
    // MARK: >> 单例化
    class func shareInstance() -> ZGCImageDBManager{
        struct psSingle{
            static var onceToken: dispatch_once_t = 0;
            static var instance: ZGCImageDBManager? = nil
        }
        //保证单例只创建一次
        dispatch_once(&psSingle.onceToken,{
            psSingle.instance = ZGCImageDBManager()
        })
    ...
    func deleteImage(c:Image) {
        ...
        if !self.dbBase.executeUpdate("delete from T_Image where pid = (?)", withArgumentsInArray: [c.pid!]) {
